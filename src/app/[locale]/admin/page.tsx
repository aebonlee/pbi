...
function AdminDashboard() {
  ...
  const [stats, setStats] = useState<Stats>({ totalUsers: 0, todaySignups: 0, weeklyActive: 0, totalVisits: 0 });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    let query = supabase
      .from("user_profiles")
      .select("id, email, display_name, created_at, last_sign_in_at, signup_domain, visited_sites", { count: "exact" })
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [totalRes, todayRes, weeklyRes] = await Promise.all([
      ...
      supabase.from("user_profiles").select("id", { count: "exact", head: true }).gte("last_sign_in_at", weekAgo.toISOString()),
    ]);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-gray-600">최근 로그인</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600">가입 도메인</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600">방문 사이트</th>
                  </tr>
                </thead>
        ...
        {activeTab === "stats" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              <p className="text-3xl font-bold text-blue-600 mt-2">{stats.todaySignups.toLocaleString()}</p>
              <p className="text-xs text-gray-400 mt-1">오늘 신규 가입</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <p className="text-sm font-medium text-gray-500">주간 활성</p>
              <p className="text-3xl font-bold text-green-600 mt-2">{stats.weeklyActive.toLocaleString()}</p>
              <p className="text-xs text-gray-400 mt-1">최근 7일 로그인</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <p className="text-sm font-medium text-gray-500">전체 방문</p>
              <p className="text-3xl font-bold text-purple-600 mt-2">{stats.totalVisits.toLocaleString()}</p>
