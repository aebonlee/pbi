            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                {accountBlock.suspended_until && (
                  <p className="text-xs text-red-500 mt-1">해제 예정: {accountBlock.suspended_until}</p>
                )}
              </div>
              <button onClick={clearAccountBlock} className="text-red-400 hover:text-red-600 text-lg leading-none">&times;</button>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          ...
          {!showResetForm && (
            <div className="flex border-b border-gray-200">
              ...
              <button
                onClick={() => { setTab("register"); setLocalError(null); setSuccessMsg(null); }}
                className={`flex-1 py-3.5 text-sm font-semibold transition-colors ${
                  tab === "register"
                    ? "text-blue-600 border-b-2 border-blue-600 bg-blue-50/50"
                    : "text-gray-500 hover:text-gray-700"
          ...
          <div className="p-6">
            ...
            {showResetForm ? (
              <form onSubmit={handleResetPassword} className="space-y-4">
                ...
                <p className="text-sm text-gray-500 mb-4">가입하신 이메일을 입력하시면 재설정 링크를 보내드립니다.</p>
                <div>
                ...
                <button
                  type="button"
                  onClick={() => { setShowResetForm(false); setLocalError(null); }}
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <button
                  type="button"
                  onClick={() => { setShowResetForm(true); setLocalError(null); setSuccessMsg(null); }}
